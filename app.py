        ...
        if not (GROQ_API_KEY and SERPER_API_KEY):
            ...
            if not resultados: st.warning("Nenhum sinal encontrado.")
                    ...
                    for future in concurrent.futures.as_completed(future_to_item):
                        try:
                            ...
                            data_export.append({
                                ...
                                "Produto": analise_data.get('produto_recomendado'), "Argumento": analise_data.get('argumento_venda')
                            })
                        except Exception as exc: st.error(f"Erro no processamento: {exc}")
                        completed += 1
                ...
                if data_export:
                    ...
                    st.download_button(label="📥 BAIXAR RELATÓRIO DO RADAR (EXCEL)", data=to_excel(df_radar), file_name="radar_leanttro.xlsx")
                for p in processed_results:
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <div>
                            ...
                            <span class="tag-nicho">Autor: {analise.get('autor', 'Desconhecido')}</span>
                        </div>
                        <a href="{p['link']}" target="_blank" style="background:#222; color:#fff; padding:5px 10px; text-decoration:none; border-radius:4px; font-size:12px;">VER POST 🔗</a>
                    </div>
                    <div style="margin-top:10px;">
                        <a href="{p['link']}" target="_blank" class="lead-title">{p['titulo']}</a>
                    </div>
                    <div style="color:#666; font-size:11px; margin-bottom:5px;">🕒 {p['data_pub']} | {p['snippet'][:200]}...</div>
...
with tab2:
    ...
    bairros_txt = st.text_area("Lista de Bairros (Cole aqui separados por vírgula):", value="Centro, Pinheiros, Vila Madalena, Moema, Tatuapé, Mooca, Itaquera, Santana, Barra Funda, Lapa, Morumbi", height=100)
    ...
    with c_btn1:
        if st.button("🚀 INICIAR VARREDURA POR ZONA", key="btn_zap_mine"):
            if not SERPER_API_KEY: st.error("Cadê a API Key do Serper, dev?")
            else:
                ...
                for i, bairro in enumerate(lista_bairros):
                    ...
                    queries = [f'site:instagram.com "{nicho}" "{bairro}" "{cidade}" "whatsapp"', f'site:facebook.com "{nicho}" "{bairro}" "{cidade}" "fale conosco"', f'"{nicho}" "{bairro}" "{cidade}" "whatsapp: 55"']
                    for q in queries:
                        ...
                        for r in resultados:
                            ...
                            if zap:
                                ...
                                if not exists_global and not exists_local:
                                    novos_leads.append({"Empresa": limpar_nome_insta(r.get('title', '')), "Nicho": nicho, "Bairro": bairro, "Whatsapp": zap, "Email": email if email else "N/D", "Link": r.get('link'), "Snippet": r.get('snippet')[:100]})
                ...
                else: st.warning("Nada encontrado nestes bairros.")
    if st.session_state["leads_zap"]:
        ...
        with c_down: st.download_button("📥 BAIXAR BASE (EXCEL)", data=to_excel(df), file_name="leads_zap_bairros.xlsx")
        with c_fire:
            if st.button("🔥 DISPARAR CAMPANHA (VIA IP EXTERNO:3001)"):
                ...
                for idx, row in df.iterrows():
                    try:
                        ...
                        payload = {"number": row['Whatsapp'], "message": f"Opa {primeiro_nome}, tudo bem? Vi que vocês atendem no {row['Bairro']}. Tentei achar o site de vcs no Google e não consegui, tá fora do ar?"}
