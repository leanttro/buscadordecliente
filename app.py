        ...
        if not (GROQ_API_KEY and SERPER_API_KEY):
            ...
            if not resultados:
                ...
                if data_export:
                    ...
                    st.download_button(label="📥 BAIXAR RELATÓRIO DO RADAR (EXCEL)", data=to_excel(df_radar), file_name="radar_leanttro.xlsx", key="dl_radar")
...
# ==============================================================================
# ABA 2: MINERADOR SNIPER (B2B + WHATSAPP)
# ==============================================================================
with tab2:
            ...
            if st.button("🔥 DISPARAR CAMPANHA (VIA NODE.JS LOCAL)"):
                ...
                for idx, row in df.iterrows():
                    try:
                        ...
                        payload = {"number": row['Whatsapp'], "message": mensagem_fria}
                        
                        # FIX SSL: verify=False + try/except robusto
                        res = requests.post("https://wppapi.leanttro.com/disparar", json=payload, timeout=8, verify=False)
                        
                        if res.status_code == 200:
                            sucessos += 1
                        else:
                            st.warning(f"Erro {res.status_code} para {row['Whatsapp']}")
                            erros += 1
                    except Exception as e:
                        # Agora o erro real aparece no Streamlit para diagnóstico
                        st.error(f"Falha Crítica em {row['Whatsapp']}: {str(e)}")
                        erros += 1
